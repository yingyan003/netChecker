...
func (p *streamProtocolV2) copyStdin() {
	if p.Stdin != nil {
		...
		go func() {
			...
			// if n, err := io.Copy(p.remoteStdin, p.Stdin); err != nil {
			n, err := io.Copy(p.remoteStdin, p.Stdin)
				fmt.Printf("streamProtocolV2 copyStdin: %d\n", n)
				var str []byte
				_, errTest := p.remoteStdin.Write(str)
				if errTest != nil {
					fmt.Printf("streamProtocolV2 copyStdin show remoteStdin error: %v\n", errTest)
				}
				fmt.Printf("streamProtocolV2 copyStdin show remoteStdin: %s\n", string(str))
			if err != nil {
				fmt.Printf("streamProtocolV2 copyStdin error: %v\n", err)
...
func (p *streamProtocolV2) copyStdout(wg *sync.WaitGroup) {
	...
	go func() {
		...
		n, err := io.Copy(p.Stdout, p.remoteStdout);
		fmt.Printf("streamProtocolV2 copyStdout: %d\n", n)
		if err != nil {
			var str []byte
			_, errTest := p.Stdout.Write(str)
			if errTest != nil {
				fmt.Printf("streamProtocolV2 copyStdout error: %v\n", errTest)
			}
			fmt.Printf("streamProtocolV2 copyStdout show stdout: %s\n", string(str))
			runtime.HandleError(err)
		}
	}()
}
...
func (p *streamProtocolV2) copyStderr(wg *sync.WaitGroup) {
	...
	go func() {
		...
		_, err := io.Copy(p.Stderr, p.remoteStderr);
		if err != nil {
			var str []byte
			_, errTest := p.Stderr.Write(str)
			if errTest != nil {
				fmt.Printf("streamProtocolV2 copyStderr show stderr error: %v\n", errTest)
			}
			fmt.Printf("streamProtocolV2 copyStderr show stderr: %s\n", string(str))
			runtime.HandleError(err)
		}
	}()
}
